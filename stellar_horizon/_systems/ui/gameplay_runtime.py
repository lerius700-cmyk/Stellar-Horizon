...
from stellar_horizon._systems.core.settings import (
...
from stellar_horizon._systems.entities.enemies import EnemyKind, EnemyPool, Enemy
from stellar_horizon._systems.entities.enemies.boss import Boss, BossId, BossPool, BOSS_CONFIGS
from stellar_horizon._systems.entities.boss_spear import BossSpear
from stellar_horizon._systems.entities.player import Player, PlayerState
from stellar_horizon._systems.systems.hitstop import Hitstop
from stellar_horizon._systems.systems.parallax import ParallaxBackground
...
from stellar_horizon._systems.systems.projectile import (
...
from stellar_horizon._systems.systems.scoring_system import ScoringSystem
from stellar_horizon._systems.systems.screen_shake import ScreenShake
from stellar_horizon._systems.systems.slowmo import SlowMo
from stellar_horizon._systems.systems.tron_trail import TronTrail
from stellar_horizon._systems.systems.weapon_system import WeaponLevel, WeaponPath, WeaponSystem
from stellar_horizon._systems.ui.hud import HUD
from stellar_horizon._systems.fx.bomb_burst import BombBurst

if TYPE_CHECKING:
    from stellar_horizon._systems.audio.synth import AudioEngine
    from stellar_horizon._systems.systems.wave_manager import BossTrigger, WaveChain, WaveManager
    from stellar_horizon._systems.ui.scenes import TransitionFn
...
from stellar_horizon._systems.core.settings import MAX_ENEMIES_ON_SCREEN as WAVE_MAX_LIVE
...
class GameplayRuntime:
    ...
    def __init__(self, transition_to: "TransitionFn", is_boss: bool = False, act: int = 1,
        ...
        from stellar_horizon._systems.ui.scrolling_galaxy import ScrollingGalaxyBackground
        ...
        from stellar_horizon._systems.ui.tiling_image import TilingImage
        ...
        from stellar_horizon._systems.entities.asteroid import Asteroid, Powerup as AsteroidPowerup
        ...
        if surf is not None:
            ...
            from stellar_horizon._systems.core.settings import WINDOW_H, WINDOW_W
    ...
    def _build_wave_manager(self) -> "WaveManager":
        from stellar_horizon._systems.systems.wave_manager import WaveManager
        return WaveManager()
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LSHIFT:
                    ...
                    from stellar_horizon._systems.core.scene_manager import GameState
    ...
    def _spawn_homing_missile(self) -> None:
        ...
        from stellar_horizon._systems.core.settings import MISSILE_LIFE_S
        ...
        if dlen > 0:
            dx /= dlen
            dy /= dlen
        from stellar_horizon._systems.core.settings import MISSILE_SPEED_PX_S
        m = HomingMissile(
            x=self._player.x,
            y=self._player.y,
    ...
    def _explode_missile(self, m: HomingMissile, big: bool, hit_x: float = 0.0, hit_y: float = 0.0) -> None:
        ...
        from stellar_horizon._systems.systems.particle_engine import P_FIRE, P_GLOW, P_SMOKE, P_SPARK
        if not m.active:
            return
        m.active = False
        cx, cy = m.x, m.y
    ...
    def _populate_spawn_queue(self) -> None:
        ...
        if formation is not None:
            from stellar_horizon._systems.systems.wave_manager import spawn_formation
    ...
    def _populate_level1_queue(self) -> None:
        ...
        """
        from stellar_horizon._systems.systems.wave_manager import WaveChain, BossTrigger
        from stellar_horizon._systems.core.settings import MAX_ENEMIES_ON_SCREEN
    ...
    def _spawn_sub_boss(self) -> None:
        ...
        """
        import random
        from stellar_horizon._systems.entities.enemies.enemy import EnemyKind
        from stellar_horizon._systems.core.settings import INTERNAL_W
        ...
        if e is not None:
            ...
            try:
                from stellar_horizon._systems.audio.synth import get_audio
            ...
            from stellar_horizon._systems.systems.particle_engine import P_SPARK, P_GLOW
            for _ in range(18):
    ...
    def _update_asteroids_and_powerups(self, dt: float) -> None:
        ...
        """
        from stellar_horizon._systems.entities.asteroid import (
    ...
    def _draw_asteroids_and_powerups(self, target: pygame.Surface) -> None:
        ...
        from stellar_horizon._systems.entities.asteroid import draw_asteroid
    ...
    def _asteroid_bullet_collision(self) -> None:
        ...
        from stellar_horizon._systems.entities.asteroid import Powerup, PowerupKind
        for ast in self._asteroids:
            if not ast.active:
                continue
    ...
    def _apply_powerup(self, kind) -> None:
        ...
        from stellar_horizon._systems.entities.asteroid import PowerupKind
    ...
    def _attach_wave_path(
        ...
        """
        from stellar_horizon._systems.movement import (
        ...
        if kind == "straight":
            ...
            from stellar_horizon._systems.movement.hybrid import HybridPath
            start = Point(e.x, e.y)
    ...
    def _update_enemies(self, dt: float) -> None:
        ...
        if self._is_boss and self._boss is not None and self._boss.active:
            ...
            if self._boss_entry_t < 1.5:
                ...
                from stellar_horizon._systems.entities.enemies.boss import BOSS_CONFIGS
                cfg = BOSS_CONFIGS[self._boss.id]
    ...
    def _spawn_enemy_bullet(self, e: Enemy) -> None:
        from stellar_horizon._systems.entities.enemies.enemy import ENEMY_CONFIGS
    ...
    def _on_enemy_killed(self, e: Enemy) -> None:
        ...
        if e.kind == EnemyKind.SUB_BOSS:
            from stellar_horizon._systems.core.settings import SUB_BOSS_FLAT_SCORE
    ...
    def enable_procedural_patterns(
        ...
        """
        from stellar_horizon._systems.systems.wave_patterns import ProceduralWaveManager
        from stellar_horizon._systems.systems.wave_patterns.composed import SoloEnemySpawner
        from stellar_horizon._systems.systems.wave_patterns.runtime import (
    ...
    def _spawn_procedural_patterns(self, dt: float) -> None:
        ...
        if hasattr(self, "_solo_spawner") and self._solo_spawner is not None:
            from stellar_horizon._systems.systems.wave_patterns.runtime import spawn_solo_ship
    ...
    def _update_level1_wave_state(self) -> None:
        ...
        """
        from stellar_horizon._systems.core.scene_manager import GameState
        from stellar_horizon._systems.core.settings import GOLIATH_SUMMON_AT_S
        chain = self._level1_chain
        ...
        if boss is not None:
            ...
            if chain.perfect:
                ...
                from stellar_horizon._systems.core.settings import PERFECT_RUN_BONUS
    ...
    def _emit_player_motion_particles(self, dt: float) -> None:
        ...
        """
        from stellar_horizon._systems.systems.particle_engine import P_DUST, P_SMOKE, P_SPARK
        px, py = self._player.x, self._player.y
    ...
    def _emit_sub_boss_propulsion(self, e: "Enemy", dt: float) -> None:
        ...
        """
        from stellar_horizon._systems.entities.enemies.enemy import ENEMY_CONFIGS
        from stellar_horizon._systems.systems.particle_engine import P_GLOW, P_SMOKE, P_SPARK
    ...
    def _spawn_tron_trail_segment(self, dt: float) -> None:
        ...
        from stellar_horizon._systems.core.settings import PLAYER_SPRITE_SCALE
        import math as _m
    ...
    def _update_tron_trail_collisions(self) -> None:
        ...
        """
        from stellar_horizon._systems.core.settings import TRON_TRAIL_DAMAGE_MULT
        from stellar_horizon._systems.systems.weapon_system import WeaponSystem
    ...
    def _maybe_drop_powerup(self, e: Enemy) -> None:
        ...
        """
        from stellar_horizon._systems.entities.enemies.enemy import ENEMY_CONFIGS
        from stellar_horizon._systems.core.settings import GOLD_RING_DROP_CHANCE
    ...
    def draw(self, target: pygame.Surface) -> None:
        ...
        if self._use_procedural_patterns and self._active_pattern_runtime is not None:
            from stellar_horizon._systems.systems.wave_patterns.runtime import draw_leader_glows
    ...
    def _draw_reticle(self, target: pygame.Surface, ox: int, oy: int) -> None:
        ...
        from stellar_horizon._systems.core.settings import INTERNAL_W, INTERNAL_H
    ...
    def _draw_engine_flame(self, target: pygame.Surface, ox: int, oy: int) -> None:
        ...
        if speed > 30.0 and int(self._t * 60) % 3 == 0:
            ...
            from stellar_horizon._systems.systems.particle_engine import P_SPARK
    ...
    def _draw_enemy(self, target: pygame.Surface, e: Enemy, ox: int, oy: int) -> None:
        ...
        if e.kind == EnemyKind.SCOUT:
            ...
            from stellar_horizon._systems.entities.enemies.enemy import sub_boss_facing_angle
    ...
    def _get_player_sprite(self) -> "Optional[pygame.Surface]":
        ...
        from stellar_horizon._systems.ui.scenes import _load_sprite
        from stellar_horizon._systems.entities.player.player import PlayerState
        if self._player.state == PlayerState.PROPULSION:
