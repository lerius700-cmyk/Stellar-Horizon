...
The long sheets are the larger version of the 4 new laser
...
- _load_sprites populates self._laser_long_sprites keyed by weapon
  (5, 6, 7, 8) and registers the same AnimatedSprite under
  laser_NN_long in self._animated so the per-frame tick advances it.
- The sheet name mapping via WEAPON_ARCHETYPE is correct:
  weapon 5 -> laser_07_long (orange flame)
  weapon 6 -> laser_08_long (white lightning)
  weapon 7 -> laser_09_long (magenta heart)
  weapon 8 -> laser_06_long (cyan ice)
- Each sheet is 6 frames at 150x50, 12 fps.
- The render block (inside the draw call after draw_charge_aura)
  does not crash for any weapon (0..9) and respects the
  charge_time=0 no-op.
"""
...
def test_laser_long_sprites_dict_has_four_entries(scene) -> None:
    ...
    AnimatedSprite per charged weapon (5, 6, 7, 8)."""
...
def test_laser_long_sprites_uses_weapon_archetype_mapping(scene) -> None:
    ...
    From bullet.py:
        weapon 5 (orange fireball)   -> archetype 6 -> laser_07_long
        weapon 6 (white piercing)    -> archetype 7 -> laser_08_long
        weapon 7 (pink heart)        -> archetype 8 -> laser_09_long
        weapon 8 (cyan ice)          -> archetype 5 -> laser_06_long
    """
    expected_sheet_name = {
        5: "laser_07_long",
        6: "laser_08_long",
...
def test_tap_only_weapons_have_no_long_sprite(scene) -> None:
    """Weapons 0-4 and 9 are tap-only, so they have NO long sheet."""
    long_sprites = scene._laser_long_sprites
    for w in (0, 1, 2, 3, 4, 9):
        assert w not in long_sprites, f"weapon {w} should not have long sheet"
...
def test_long_sheet_advances_via_animated_update(scene) -> None:
    ...
    anim = scene._laser_long_sprites[5]
    before = anim._index
...
def test_draw_charged_weapons_does_not_crash(scene) -> None:
    ...
    for weapon in (5, 6, 7, 8):
        ...
        # Need to also call on_fire_pressed so the player's internal
        # charge_time is allowed to be > 0. (The setter only updates
        # the field directly; the state machine is OK with this in
        # the test since we don't run a full update loop.)
...
def test_alpha_ramp_reaches_full_for_discrete_weapons(scene) -> None:
    ...
    threshold for discrete weapons (6=1.2, 7=1.5) and at 0.5s for
    continuous weapons (5, 8).
    ...
    assert alpha(6, 1.2) == pytest.approx(1.0)
    assert alpha(7, 1.5) == pytest.approx(1.0)
    # Continuous weapons: full alpha at 0.5s.
    assert alpha(5, 0.5) == pytest.approx(1.0)
    assert alpha(8, 0.5) == pytest.approx(1.0)
    # Below the ramp time, alpha is partial.
    assert 0.0 < alpha(6, 0.6) < 1.0
    assert 0.0 < alpha(7, 0.75) < 1.0
    assert 0.0 < alpha(5, 0.25) < 1.0
